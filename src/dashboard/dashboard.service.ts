...
export class DashboardService {
  ...
  async getStats(): Promise<DashboardStats> {
    ...
    const [allWallets, activeWallets, trades, positions, latencyStats] =
      await Promise.all([
        this.wallets.findAll(),
        this.wallets.findActive(),
        this.copyTrading.getLeaderTrades(),
        this.copyTrading.getBotPositions(),
    ...
    const copiedLast7Days = trades.filter(
      (t) =>
        t.status === TradeStatus.COPIED &&
        new Date(t.createdAt) >= since7DaysAgo,
    ).length;
    const skipped = trades.filter((t) => t.status === TradeStatus.SKIPPED)
      .length;
  ...
  private async getCopyLatencyStats(): Promise<{
    ...
    const trades = (await this.copyTrading.getLeaderTrades()).filter(
  ...
  async getRecentTrades(
    ...
    let trades = await this.copyTrading.getLeaderTrades();
    if (onlyCopied) {
      trades = trades.filter((t) => t.status === TradeStatus.COPIED);
    }
    trades = [...trades]
      .sort(
        (a, b) =>
  ...
  async getWeeklyReports(weeks = 12): Promise<WeeklyReportDto[]> {
    const wallets = await this.wallets.findAll();
    const walletLabels = new Map(wallets.map((w) => [w.wallet, w.label]));
    const allTrades = await this.copyTrading.getLeaderTrades();
    ...
    for (let i = 0; i < weeks; i++) {
      ...
      const copied = trades.filter((t) => t.status === TradeStatus.COPIED)
        .length;
      const skipped = trades.filter((t) => t.status === TradeStatus.SKIPPED)
        .length;
      const failed = trades.filter((t) => t.status === TradeStatus.FAILED)
        .length;
  ...
  async getComparativeAnalysis(): Promise<ComparativeAnalysisDto> {
    const wallets = await this.wallets.findAll();
    const walletLabels = new Map(wallets.map((w) => [w.wallet, w.label]));
    const trades = await this.copyTrading.getLeaderTrades();
